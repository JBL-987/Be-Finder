    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Business Analysis Results - Be-Finder',
          text: `Business analysis shows potential daily revenue of ${new Intl.NumberFormat('id-ID', {
            style: 'currency',
            currency: 'IDR',
            minimumFractionDigits: 0
          }).format(analysisResults.metrics.dailyRevenue)} with ${analysisResults.metrics.tppd} customers per day.`,
      ...
      const shareText = `Business Analysis Results - Be-Finder\n\nDaily Revenue: ${new Intl.NumberFormat('id-ID', {
        style: 'currency',
        currency: 'IDR',
        minimumFractionDigits: 0
      }).format(analysisResults.metrics.dailyRevenue)}\nDaily Customers: ${analysisResults.metrics.tppd}\n\nAnalyzed with Be-Finder AI`;
    ...
    <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-purple-900/30 to-purple-800/30 rounded-lg p-4 border border-purple-800/30">
            ...
            <div className="text-xl font-bold text-purple-400">
              {new Intl.NumberFormat('id-ID', {
                ...
                minimumFractionDigits: 0,
                maximumFractionDigits: 0
              }).format(analysisResults.metrics.monthlyRevenue)}
            </div>
          </div>
      ...
      <div className="bg-dark-surface rounded-xl border border-dark-border">
        <button
          ...
          className="w-full p-4 flex items-center justify-between text-left hover:bg-dark-bg/50 transition-colors rounded-t-xl"
        >
          <h3 className="text-lg font-semibold text-white">Area Distribution Analysis</h3>
      ...
      <div className="bg-dark-surface rounded-xl border border-dark-border">
        <button
          ...
          className="w-full p-4 flex items-center justify-between text-left hover:bg-dark-bg/50 transition-colors rounded-t-xl"
        >
          <h3 className="text-lg font-semibold text-white">Business Profitability Metrics</h3>
          {showBusinessMetrics ? (
            <ChevronUp className="w-5 h-5 text-gray-400" />
          ) : (
            <ChevronDown className="w-5 h-5 text-gray-400" />
      ...
      <div className="bg-dark-surface rounded-xl border border-dark-border">
        ...
        {showRawData && (
          <div className="p-6 pt-0">
            <pre className="bg-dark-bg rounded-lg p-4 text-xs text-gray-300 overflow-x-auto border border-dark-border">
